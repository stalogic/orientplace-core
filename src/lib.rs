...
fn calc_single_wire_img(node_net_info: HashMap<String, (usize, usize, usize, usize, usize, usize, f32)>, grid: usize) -> Vec<Vec<f32>> {
    let mut wire_img: Vec<Vec<f32>> = vec![vec![0.; grid]; grid];
...
fn calc_wire_img(node_net_info: HashMap<usize, HashMap<String, (usize, usize, usize, usize, usize, usize, f32)>>, grid: usize) -> PyResult<Vec<Vec<Vec<f32>>>> {
    ...
    for orient in 0..8 {
        let img: Vec<Vec<f32>> = calc_single_wire_img(node_net_info.get(&orient).unwrap().clone(), grid);
        wire_img.push(img);
    }
...
fn calc_wire_img_parallel(node_net_info: HashMap<usize, HashMap<String, (usize, usize, usize, usize, usize, usize, f32)>>, grid: usize) -> PyResult<Vec<Vec<Vec<f32>>>> {
    ...
    let mut handles: Vec<thread::JoinHandle<Vec<Vec<f32>>>> = Vec::with_capacity(8);
    for orient in 0..8 {
        let net_info: HashMap<String, (usize, usize, usize, usize, usize, usize, f32)> = node_net_info.get(&orient).unwrap().clone();
        let new_grid: usize = grid.clone();
        let handle: thread::JoinHandle<Vec<Vec<f32>>> = thread::spawn(move || {
    ...
    for handle in handles {
        let img: Vec<Vec<f32>> = handle.join().unwrap();
        wire_img.push(img);
    }
    Ok(wire_img)
}
